...
```
Using the histograms, BARTHAG is the only plot that does not look normally distrbuted. It looks left skewed. We did a log transformation of BARTHAG, but it still appeared left skewed. Therefore, we decided to remove BARTHAG. Using the quantile plots, BARTHAG, seed, and year are not normally distributed. Seed and year were categorical numeric variables, we decided to remove those as well instead of trying transformations.
...
#total variance explained by a given number of principle components
sum(pc$sdev[1:3])/12
#38% of the variance is explained by these 3 principle components. 
...
* comp 1 = wins and efficiency (G, W, WAB, ADJOE, ADJDE, EFG_D)
  + All of these variables are positively correlated with each other, except for EFG_D. (See top of document for descriptions of each variable.)
...
* component 1 is most affected by the variables W, ADJOE, X3P_O, FTRD
* component 2 is most affected by the variables X3P_D, YEAR, ADJ_T, X2P_D
* The variables BARTHAG, G, X2P_O, EFG_O affect both component 1 and component 2.
